...
const MenuCategoryDashboard = () => {
  ...
  const handleCreateOrUpdateCategory = async () => {
    if (!formData.name.trim()) {
      showOperationStatus('error', 'Category name is required');
      return;
    }
  ...
  const handleDeleteCategory = async (category) => {
    ...
    try {
      setOperationLoading(true);
      await menuCategoryService.deleteCategory(category.id);
  ...
  const handleViewCategory = (category) => {
    ...
    navigate(`/admin/dashboard/menu-category/${category.id}`);
  };
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="text-left py-3 px-4 text-gray-600 font-semibold hidden md:table-cell">Total Items</th>
